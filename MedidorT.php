...
$ConceptoID = $_POST['Concepto'];
 $Consulta = "SELECT * from [CONTROLTOTAL(PANORAMA)].dbo.agu_ConceptosLecturas where cl_id=$ConceptoID";
                      $R = sqlsrv_query($con, $Consulta);
                      while ($Fila2 = sqlsrv_fetch_array($R)) {
                        $no=$Fila2["cl_Nombre"];
                      }
header('Content-type: application/vnd.ms-excel;charset=iso-8859-15');
header('Content-Disposition: attachment; filename=Panorama '. $no.'.xls');
header("Content-Type: text/html;charset=utf-8");
	...
	$busqueda = "SELECT inm_Id,dbo.alote(inm_id) as lote,lec_FechaOperacion,lec_Anterior,lec_Actual,cl_id,dbo.conceptolectura(cl_id) as n
	 from [CONTROLTOTAL(PANORAMA)].dbo.agu_Lecturas where mes_Id= dbo.devuelveIdPeriodoActual()-1 and cl_id=$ConceptoID";
//		and rec.inm_id = 39 
  ...
  echo "<table border='1' cellpadding='2' cellspacing='0' width='100%'>
      <caption><font color='red' size='10'>$no</font></caption>
      <tr>
        <td>Inmueble</td>
        <td>Lote</td>
