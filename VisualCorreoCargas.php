...
round( dbo.ecuentasaldopormes(inm_id,CONVERT (date, SYSDATETIME())),2)
 ...
 ,propietario from recibos where mes_id=dbo.devuelveIdPeriodoActual()
   ...
   while($fila = sqlsrv_fetch_array($buscar)){
    ...
    $pdf->Cell(60, 7, utf8_decode($propietario_), 1, 1, 'C');
  }
  $pdf->Output($filename, 'F');
  if (isset($inm_IdGenerado_)) {
...
round( dbo.ecuentasaldopormes(inm_id,CONVERT (date, SYSDATETIME())),2)
 ...
 ,propietario from recibos where mes_id=dbo.devuelveIdPeriodoActual()
   order by recibo";*/
   $Consulta="
SELECT inm_IdGenerado,recibo,fechaEmision as fecha,
round( dbo.ecuentasaldopormes(inm_id,CONVERT (date, SYSDATETIME())),2)
  as totalapagar
 ,propietario from recibos where mes_id=dbo.devuelveIdPeriodoActual()
   order by recibo";
   ...
   /* $mail->Username = 'zulygalicia4@gmail.com';   //username
    $mail->Password = 'ocefnkqoqgbxhxgn';*/
     ...
     $mail->setFrom('sasciminformatica@gmail.com', 'Sascim Informatica');
        $mail->addAddress('ccajena.coordinador2@banrural.com.gt','Byron Vicente'); 
       $mail->addAddress('cca.auxiliar9@banrural.com.gt', ''); 
       $mail->addAddress('cca.auxiliar3@banrural.com.gt', 'CCA Auxiliar3'); 
       $mail->addAddress('cca@banrural.com.gt', '');
       $mail->addBCC('intermpanorama@gmail.com', 'Panorama'); 
        /*$mail->setFrom('zulygalicia4@gmail.com', 'Sascim Informatica');
        $mail->addBCC('pasarcosasiphone1@gmail.com','zuly'); 
          ...
          <div class="panel-header panel-header-lg" >
            <div class="container container-form-lg"  >
              <div class="col-md-12 ">
                ...
                <form class="needs-validation" novalidate method="POST" action="VisualCorreoCargas.php">
